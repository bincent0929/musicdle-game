...
async function song_fetch(): Promise<ITunesRSSEntry[]> {
  ...
  const feed: ITunesRSSResponse = await fetch(
    "https://itunes.apple.com/us/rss/topsongs/limit=200/genre=1/json"
  )
    .then((r) => r.json())
    .catch((e) => ({ feed: { entry: [] } }));
  ...
  if (!entries.length)
    throw new Error("No songs found for that genre/country.");
...
async function pickSongWithPreview(tries = 6): Promise<currentSong> {
  ...
  for (let i = 0; i < tries; i++) {
    ...
    const trackId: ITunesRSSEntry["id"]["attributes"]["im:id"] =
      chosen?.id?.attributes?.["im:id"];
    if (!trackId) continue;

    const looked: ITunesSearchResponse | null = await fetch(
      ...
      .then((r) => r.json())
      .catch(() => null);
    const item: ITunesTrack | undefined =
      looked?.results?.find((x: ITunesTrack) => x.kind === "song") ||
      looked?.results?.[0];
  ...
  throw new Error(
    "Could not find a preview for any of the picked tracks. Try again."
  );
}
...
export async function searchItunesTrack(
  ...
  let guessedTrack = data.results.find(
    (t) => normalize(t.trackName) === normalizedGuess
  );
...
export function scheduleNextUpdate(
  updateCallback: (song: DailySong | null) => void
) {
  const now = new Date();
  ...
  console.log(
    `Next update scheduled in ${msUntilTarget}ms (at ${target.toISOString()})`
  );
